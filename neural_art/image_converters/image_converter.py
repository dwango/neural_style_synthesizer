...
class BaseImageConverter(object):
    ...
    def convert(self, content_img, init_img, iteration=1000):
        return self.convert_debug(content_img, init_img, max_iteration=iteration, debug_span=iteration+1, output_directory=None)
...
class ImageConverterMatrix(BaseImageConverter):
    ...
    def __init__(self, texture_img, gpu=-1, optimizer=None, model=None, content_weight=1, texture_weight=1):
        super(ImageConverterMatrix, self).__init__(gpu=gpu, optimizer=optimizer, model=model, content_weight=content_weight, texture_weight=texture_weight)
        texture_array = self.xp.array(neural_art.utility.img2array(texture_img))
        self.texture_matrices = [neural_art.utility.get_matrix(layer) for layer in self.model.forward_layers(chainer.Variable(texture_array), average_pooling=self.average_pooling)]
...
class ImageConverter(BaseImageConverter):
    ...
    """
    def __init__(self, texture_img, gpu=-1, optimizer=None, model=None, content_weight=1, texture_weight=1):
        super(ImageConverter, self).__init__(gpu=gpu, optimizer=optimizer, model=model, content_weight=content_weight, texture_weight=texture_weight)
        texture_array = self.xp.array(neural_art.utility.img2array(texture_img))
        self.texture_feature = self._to_texture_feature(self.model.forward_layers(chainer.Variable(texture_array), average_pooling=self.average_pooling))
